...
export class DemoEnvironmentGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    const isDemo = this.configService.get<boolean>('IS_DEMO_MODE') || false;
    ...
    const whitelist = ['/auth/login', '/auth/logout', '/auth/captcha'];
    ...
    if (request.method !== 'GET') {
      throw new ForbiddenException('演示环境，禁止执行修改操作！');
    }
